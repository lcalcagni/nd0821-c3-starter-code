...
def get_predicition(payload: Input):
    ...
    try:
        ...
        model = pd.read_pickle(os.path.join(path,"model/model.pkl").replace('starter/', ''))
        Encoder = pd.read_pickle(os.path.join(path,"model/encoder.pkl").replace('starter/', ''))
        lb_ = pd.read_pickle(os.path.join(path,"model/lb.pkl").replace('starter/', ''))
